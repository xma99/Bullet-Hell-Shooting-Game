...
namespace Alpha_Danmaku_Rush_Demo.Src.Entities.Player
{
    public class PlayerAttackDecorator : IPlayer
    {
        ...
        public PlayerAttackDecorator(IPlayer player, float attackSpeed, EnemyManager enemyManager)
        {
        ...
        private void PerformAttack()
        {
            ...
            newBullet = new PlayerBullet(content.Load<Texture2D>("bubble"), Position, BulletFactory.AdjustVelocity(new Vector2(0, 1), -10), ColorHelper.FromName("white"));
            newBullet.Speed = 10;
        }

        private void PerformBomb()
        {
            // The bombs will clear all the enemies
            if (_isAttacking)
            {
                return;
            }
            _isAttacking = true;
            newBullet = new PlayerBullet(content.Load<Texture2D>("bomb"), Position, BulletFactory.AdjustVelocity(new Vector2(0, -1), 10), Color.White);
            enemyManager.Bomb();
        }
